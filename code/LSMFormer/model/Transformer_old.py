...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.drop_path(self.mlp(self.norm1(x)))  # self.norm1(x)
        return x
...
class Transformer_old(nn.Module):
    def __init__(self,
                 ...
                 qkv_bias=True,
                 attn_drop_ratio=0.3,
                 drop_path_ratio=0.3, init_weights=True):
        super(Transformer_old, self).__init__()
        backbone = []

        for i in range(depth):
            backbone.append(Block(dim=factors,
                                  num_heads=num_heads,
                                  qkv_bias=qkv_bias,
        ...
        self.head = nn.Sequential(nn.Linear(factors, 64),
                                  nn.GELU(),
...
if __name__ == '__main__':
    input = torch.rand([500, 36])  # torch.float32
    block = Transformer_old(input.shape[-1], 0.2)
