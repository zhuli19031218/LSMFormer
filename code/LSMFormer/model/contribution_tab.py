...
class Con_Transformer(nn.Module):
    def __init__(self,  dim, in_features, num_tokens, depth, heads, dim_head, attn_dropout, ff_dropout):
        super().__init__()
        num_tokens=24
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, mlp_ratio=4.0, act_layer=nn.GELU, drop_ration=0.3):
        super().__init__()
    ...
    def __init__(
        ...
        *,
        ...
        self.cate_embedding = nn.Sequential(
            nn.Linear(in_features=self.num_categories, out_features=self.num_categories * dim)
        )
        ...
        self.transformer2 = Con_Transformer(  # 包含编码
            # num_tokens=total_tokens,
        ...
        self.final_transformer = Final_Transformer(  # 包含编码
            # num_tokens=total_tokens,
            in_features=self.num_continuous,
            num_tokens=24,
            dim=self.num_categories * dim + self.num_continuous,
            depth=depth,
            heads=heads,
            dim_head=dim_head,
            attn_dropout=attn_dropout,
        ...
        self.mlp = Mlp(in_features=(dim*self.num_categories)+num_continuous)
        # self.apply(_init_weight)

    def forward(self,  input):  # input: tensor

        x_cont = input[:, :18]
        x_categ = input[:, 18:]  # , dtype=torch.int64
        ...
        x_categ = torch.tensor(x_categ, dtype=torch.float32).to("cuda")
        x_cont = torch.tensor(x_cont, dtype=torch.float32).to("cuda")

        x_categ = self.cate_embedding(x_categ)
        normed_cont = self.norm(x_cont)
        x = torch.cat((x_categ, normed_cont), dim=-1)
        x = self.final_transformer(x)
        print("final", x.shape)
        x = torch.squeeze(input=x, dim=1)
        ...
        # un_x = normed_cont
        # un_x = torch.unsqueeze(normed_cont, dim=1)
        # print('un_x前', un_x.shape)
        # un_x = self.transformer2(un_x)
        # print('un_x后', un_x.shape)
        # un_x = torch.squeeze(input=un_x, dim=1)
        # print('un_x squeeze', un_x.shape)

        # x = torch.cat((flat_categ, normed_cont), dim = -1)
        # x = torch.cat((flat_categ, un_x), dim=-1)
        # print('concat后x.shape',x.shape)

        x = self.mlp(x)

        return x.detach().cpu().numpy()     # .detach().numpy()

if __name__ == '__main__':
    transformer_cfg = {
        'categories': [1] * 3,  # iterable with the number of unique values for categoric feature
        'num_continuous': 18,  # continuous dimensions in data
